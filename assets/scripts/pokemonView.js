var PokemonView = Backbone.View.extend({
	...
	renderBars: function() {
		...
		//set hex to rgb
		//get brightness
		//if its less than 130 make it white
		var color = colors[2][0]
		if ( 130 > Math.sqrt(parseInt(color.substr(1, 2), 16) * parseInt(color.substr(1, 2), 16) * .241 + parseInt(color.substr(3, 2), 16) * parseInt(color.substr(3, 2), 16) * .691 +  parseInt(color.substr(5, 2), 16) *  parseInt(color.substr(5, 2), 16) * .068)) {
			console.log("test")
			$(this.text_color)
			.css('color', 'white')
		};
